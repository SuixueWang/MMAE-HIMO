...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=1024, patch_size=16, in_chans=3,
        ...
        self.encoder_omics = nn.Linear(80, embed_dim, bias=True)
        ...
        self.decoder_omics = nn.Linear(decoder_embed_dim, 80, bias=True)  # decoder to omics
        # --------------------------------------------------------------------------
    ...
    def forward_encoder(self, img, X_mrna, X_mirna, X_meth, X_path, mask_ratio):
        # embed patches
        ...
        X_path = self.encoder_omics(X_path).unsqueeze(1)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore.shape[1] + 1 + num_omics - x.shape[1], 1)
        x_ = torch.cat([x[:, 1+num_omics:, :], mask_tokens], dim=1)  # no cls token
        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
        x = torch.cat([x[:, :1+num_omics, :], x_], dim=1)  # append cls token
    ...
    def forward_loss_omics(self, pred_omics, X_mrna, X_mirna, X_meth, X_path):

        X_omics = torch.cat((X_mrna.unsqueeze(1), X_mirna.unsqueeze(1),
                             X_meth.unsqueeze(1), X_path.unsqueeze(1)), dim=1)

        loss = (pred_omics - X_omics) ** 2
        loss = loss.mean()

        return loss

    def forward(self, img, X_mrna, X_mirna, X_meth, X_path, mask_ratio=0.75):
        latent, mask, ids_restore = self.forward_encoder(img, X_mrna, X_mirna, X_meth, X_path, mask_ratio)
        pred_omics, pred_image = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss_image = self.forward_loss_image(img, pred_image, mask)
        loss_omics = self.forward_loss_omics(pred_omics, X_mrna, X_mirna, X_meth, X_path)

        loss = loss_omics * 0.4 + loss_image * 0.6
